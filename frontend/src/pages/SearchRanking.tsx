...
const SearchRanking: React.FC = () => {
  ...
  const [rankingType, setRankingType] = useState<string>('title'); // Remplacé 'occurrences' par 'title' comme critère par défaut
  ...
  const handleSearch = (searchQuery: string) => {
    ...
    if (rankingType === 'title') {
      sortedResults = [...mockResults].sort((a, b) => a.title.localeCompare(b.title));
    } else {
      // Si tu veux une autre logique de classement (par exemple, longueur du contenu), tu peux l'ajouter ici
      sortedResults = [...mockResults].sort((a, b) => b.content.length - a.content.length);
    }
              ...
              <div className="flex justify-center space-x-8">
                <label className="flex items-center space-x-2 text-teal-700 font-medium">
                  <input
                    type="radio"
                    value="title"
                    checked={rankingType === 'title'}
                    onChange={() => setRankingType('title')}
                ...
                <label className="flex items-center space-x-2 text-teal-700 font-medium">
                  <input
                    ...
                    value="contentLength"
                    checked={rankingType !== 'title'}
                    onChange={() => setRankingType('contentLength')}
                    className="text-teal-600 focus:ring-teal-500"
                  />
